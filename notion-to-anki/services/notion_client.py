...
class NotionClient:
    ...
    def create_page(self, database_id, data: dict):
    ...
    def get_block_tree(self, block_id, page_size=100):
        """
        """
        ...
        for child in children:
            if child["has_children"]:
                child["children"] = self.get_block_tree(child["id"])
            else:
                child["children"] = []

        return children

    def get_all_page_content(self, page_id):
        blocks = self.get_page_blocks(page_id)
        for block in blocks:
            if block.get("has_children", False):
                block["children"] = self.get_block_tree(block["id"])
            else:
                block["children"] = []
        return blocks
    ...
    # ------------------------------ Notion2Anki ------------------------------
    def block_to_html(self, block, indent=""):
        """
        ...
        if block["type"] == "paragraph":
            ...
            for child in block.get("children", []):
                block_html += (self.block_to_html(child, indent + "  ")) + "\n"
